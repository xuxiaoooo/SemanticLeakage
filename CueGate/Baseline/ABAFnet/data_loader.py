...
"""
ABAFnet数据加载器
使用Librosa提取Spectrogram和mel-Spectrogram特征
"""
...
class AudioDataset(Dataset):
    """音频数据集类，提取Spectrogram和mel-Spectrogram"""
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            spectrogram = np.abs(spectrogram) ** 2  # 功率谱
            spectrogram = librosa.power_to_db(spectrogram, ref=np.max)  # 转换为dB
            ...
            # 转换为tensor并添加通道维度
            # 形状: (freq_bins, time_frames) -> (1, freq_bins, time_frames)
            spectrogram = torch.FloatTensor(spectrogram).unsqueeze(0)
            mel_spectrogram = torch.FloatTensor(mel_spectrogram).unsqueeze(0)
            
            # 如果尺寸不一致，进行裁剪或填充（取较小的尺寸）
            # 确保两个特征的时间维度一致
            min_time = min(spectrogram.shape[2], mel_spectrogram.shape[2])
            spectrogram = spectrogram[:, :, :min_time]
            mel_spectrogram = mel_spectrogram[:, :, :min_time]
            
            return spectrogram, mel_spectrogram, torch.FloatTensor([label])
            
        except Exception as e:
            print(f"加载音频文件失败: {audio_path}, 错误: {e}")
            # 返回零特征作为备选
            # 计算正确的形状
            time_frames = self.segment_samples // self.hop_length + 1
            freq_bins = self.n_fft // 2 + 1
            dummy_spec = torch.zeros((1, freq_bins, time_frames))
            dummy_mel_spec = torch.zeros((1, self.n_mels, time_frames))
            return dummy_spec, dummy_mel_spec, torch.FloatTensor([label])
    ...
    def _load_data(self):
        ...
        for _, row in questionnaire_df.iterrows():
            ...
            if os.path.exists(audio_path):
                ...
                print(f"警告: 音频文件不存在 (participant {audio_id}): {audio_path}")
...
if __name__ == "__main__":
    # 测试数据加载器
    audio_dir = "/home/a001/xuxiao/SemanticLeakage/ACL/pre_test/p2_Clean"
    questionnaire_path = "/home/a001/xuxiao/SemanticLeakage/data/DAIC/Detailed_PHQ8_Labels.csv"
    
    # 创建数据加载器
    data_loader = ABAFnetDataLoader(
        audio_dir=audio_dir,
    ...
    # 测试一个批次
    for batch_idx, (spec, mel_spec, labels) in enumerate(train_loader):
        print(f"批次 {batch_idx}: Spectrogram形状 {spec.shape}, Mel-Spectrogram形状 {mel_spec.shape}, 标签形状 {labels.shape}")
        break
