"""
Cue Detection Analysis for E-DAIC dataset.
...
# 路径配置
OUTPUTS_DIR = Path(__file__).parent.parent.parent / "agent" / "outputs" / "E-DAIC"
...
def load_all_cue_detections():
    ...
    results = []
    for sample_dir in sorted(OUTPUTS_DIR.glob("*_AUDIO")):
        cue_file = sample_dir / "cue_detection.json"
        if cue_file.exists():
            with open(cue_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                data["sample_id"] = sample_dir.name.replace("_AUDIO", "")
                results.append(data)
    return results
