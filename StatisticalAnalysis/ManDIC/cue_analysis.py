"""
Cue Detection Analysis for ManDIC dataset.
...
# 路径配置
OUTPUTS_DIR = Path(__file__).parent.parent.parent / "agent" / "outputs" / "ManDIC"
...
def load_all_cue_detections():
    ...
    results = []
    # ManDIC 输出目录命名格式: NN_XXXXXXXX_SXXX
    for sample_dir in sorted(OUTPUTS_DIR.glob("NN_*")):
        if sample_dir.is_dir():
            cue_file = sample_dir / "cue_detection.json"
            if cue_file.exists():
                with open(cue_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    data["sample_id"] = sample_dir.name
                    results.append(data)
    return results
...
def load_all_durations():
    ...
    for sample_dir in sorted(OUTPUTS_DIR.glob("NN_*")):
        if sample_dir.is_dir():
            preprocess_file = sample_dir / "preprocess_spans.json"
            if preprocess_file.exists():
                with open(preprocess_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    durations.append({
                        "sample_id": sample_dir.name,
                        "duration_sec": data.get("original_duration", 0)
                    })
    return durations
...
def analyze_durations(durations):
    ...
    return {
        ...
        "mean_duration_sec": np.mean(dur_values),
        "median_duration_sec": np.median(dur_values),
        "std_duration_sec": np.std(dur_values),
        "min_duration_sec": min(dur_values),
        "max_duration_sec": max(dur_values),
    }
...
def plot_detection_distribution(coverage_data, output_path):
    ...
    # 计算直方图数据
    max_cues = max(cue_counts) if cue_counts else 0
    if max_cues == 0:
        print("No cues detected in any sample")
        return
...
1. OVERVIEW
   ...
   - Samples with detected cues: {samples_with_cues} ({samples_with_cues/total_samples*100:.1f}% if total_samples > 0 else 0)
   - Samples without cues: {total_samples - samples_with_cues} ({(total_samples-samples_with_cues)/total_samples*100:.1f}% if total_samples > 0 else 0)

2. CUE STATISTICS
   - Total cue instances detected: {total_cues}
   - Unique depression-related words: {unique_words}
   - Average cues per sample: {total_cues/total_samples:.2f if total_samples > 0 else 0}
...
3. TIME COVERAGE
   ...
   - Average cue time per sample: {np.mean(cue_times):.2f if cue_times else 0} seconds
   - Average coverage ratio: {np.mean(cue_ratios):.3f if cue_ratios else 0}%
   - Max coverage ratio: {max(cue_ratios):.3f if cue_ratios else 0}%
...
5. ALL DETECTED WORDS ({unique_words} unique)
   {', '.join(sorted(word_counts.keys())) if word_counts else 'None'}
...
def main():
    ...
    duration_stats = analyze_durations(durations)
    
    if duration_stats:
        print("\n" + "="*80)
        print("DATASET DURATION STATISTICS")
        print("="*80)
        print(f"总样本数: {duration_stats['total_samples']}")
        print(f"总时长: {duration_stats['total_duration_sec']:.2f} 秒 = {duration_stats['total_duration_min']:.2f} 分钟 = {duration_stats['total_duration_hours']:.2f} 小时")
        print(f"平均时长: {duration_stats['mean_duration_sec']:.2f} 秒")
        print(f"中位数时长: {duration_stats['median_duration_sec']:.2f} 秒")
        print(f"标准差: {duration_stats['std_duration_sec']:.2f} 秒")
        print(f"最短时长: {duration_stats['min_duration_sec']:.2f} 秒")
        print(f"最长时长: {duration_stats['max_duration_sec']:.2f} 秒")
        print("="*80)
