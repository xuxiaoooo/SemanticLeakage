...
def train_abafnet(
    ...
    # 创建特征提取模型
    spec_model = SpectrogramModel(activation=nn.ReLU())
    mel_spec_model = MelSpectrogramModel(activation=nn.ReLU())
    
    # 创建融合模型
    model = TwoFeatureFusionModel(
        spec_model=spec_model,
        mel_spec_model=mel_spec_model,
        lstm_input_size=256,
        hidden_size=256,
        num_heads=4,
        weight_spec=0.5,
    ...
    for epoch in range(max_epochs):
        ...
        with torch.no_grad():
            for spec, mel_spec, labels in val_loader:
                spec = spec.to(device)
                mel_spec = mel_spec.to(device)
                labels = labels.to(device)
                ...
                # 收集预测和标签
                all_val_preds.extend(outputs.detach().cpu().numpy().flatten())
    ...
    with torch.no_grad():
        for spec, mel_spec, labels in test_loader:
            spec = spec.to(device)
            mel_spec = mel_spec.to(device)
...
def main():
    ...
    # 创建保存目录
    save_dir = os.environ.get('SAVE_DIR', str(base_root / "runs" / "ABAFnet"))
    ...
    # 训练模型
    model, task_results = train_abafnet(
        audio_dir=audio_dir,
        questionnaire_path=questionnaire_path,
        target_task=task,
